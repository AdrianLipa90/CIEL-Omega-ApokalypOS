    ...
    with ui.tab_panel('Models'):
        with ui.row().classes('w-full p-4 gap-4'):
            with ui.card().classes('w-1/2'):
                ...
                # ── Dodaj z dysku ──────────────────────────────────────────
                ui.separator()
                ui.label('Add from disk').classes('text-sm font-semibold text-gray-500')
                disk_path_input = ui.input(
                    'Local path to .gguf file',
                    placeholder='/home/user/models/llama3.gguf'
                ).classes('w-full')

                def _add_from_disk() -> None:
                    raw = (disk_path_input.value or '').strip()
                    p = Path(raw).expanduser().resolve()
                    if not raw:
                        ui.notify('Enter file path', type='warning')
                        return
                    if not p.exists():
                        ui.notify(f'File not found: {p}', type='negative')
                        return
                    if not p.suffix.lower() == '.gguf':
                        ui.notify('File must end with .gguf', type='warning')
                        return
                    _add_model(p.name, str(p))
                    ui.notify(f'Registered: {p.name}')
                    disk_path_input.value = ''
                    refresh_models()

                with ui.row().classes('w-full justify-end gap-2 mb-2'):
                    ui.button('Register from disk', on_click=_add_from_disk)

                # ── Download z URL ─────────────────────────────────────────
                ui.separator()
                ui.label('Download from URL').classes('text-sm font-semibold text-gray-500')
...
_FAVICON = str(Path(__file__).resolve().parent / 'main' / 'Logo1.png')
ui.run(title='CIEL/Ω', reload=False, favicon=_FAVICON)
