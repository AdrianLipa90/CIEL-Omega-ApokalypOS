"""
CIEL / TIR Information Dynamics v1
...
Purpose
-------
Close the information-dynamics layer as an executable conservation system.
...
J0 is not a magic constant. It is represented explicitly as declared sectors:
...
Important epistemic separation:
    - continuity mechanics: IMPLEMENTED / TESTED
    - sector bookkeeping: IMPLEMENTED / TESTED
    - radial Green/Kepler source sector on B3: CONDITIONAL_EXACT
    - physical identification of every J0 component in TIR runtime: OPEN
"""
...
class InformationFieldState:
    ...
    def __post_init__(self):
        ...
        if self.sectors.total_J0.shape != J.shape:
            raise ValueError("J0 sector shape mismatch")
        if self.dx <= 0:
    ...
    def total_current(self) -> np.ndarray:
        return self.phase_current + self.sectors.total_J0
...
def cell_to_face_flux(J: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if nx > 1:
        Fx[1:nx] = 0.5 * (J[:-1, :, :, 0] + J[1:, :, :, 0])
    if ny > 1:
        Fy[:, 1:ny] = 0.5 * (J[:, :-1, :, 1] + J[:, 1:, :, 1])
    if nz > 1:
        Fz[:, :, 1:nz] = 0.5 * (J[:, :, :-1, 2] + J[:, :, 1:, 2])
    return Fx, Fy, Fz
...
def continuity_step_from_faces(
    ...
    new_state = InformationFieldState(
        rho=rho_new,
    ...
    return new_state, ContinuityReceipt(
        dt=float(dt),
        before_information=before,
        after_information=after,
        source_integral=src,
        boundary_outflow=bout,
        balance_residual=float(residual),
        max_local_residual=float(np.max(np.abs(local))),
    )
...
def classify_declared_J0(sectors: CurrentSectors) -> J0Classification:
    return J0Classification(True,True,True,True,"EXPLICIT_SECTOR_DECLARATION")
...
def snapshot(state: InformationFieldState) -> InformationDynamicsSnapshot:
    return InformationDynamicsSnapshot(
        ...
        phase_current_norm=float(np.linalg.norm(state.phase_current)),
        J0_norm=float(np.linalg.norm(state.sectors.total_J0)),
        total_current_norm=float(np.linalg.norm(state.total_current)),
    )
...
__all__ = [
    ...
    "J0Classification","classify_declared_J0","InformationDynamicsSnapshot","snapshot",
]
