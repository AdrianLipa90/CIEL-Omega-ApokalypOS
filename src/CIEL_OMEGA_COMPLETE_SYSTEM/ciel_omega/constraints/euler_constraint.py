...
def _build_affect_phase_vector(metadata: Mapping[str, Any] | None = None) -> np.ndarray:
    ...
    # Center on 0: neutral (0.5) → 0 rad, high → +π, low → -π
    phases = [
        _wrap(2.0 * math.pi * (salience - 0.5)),
        _wrap(2.0 * math.pi * (confidence - 0.5)),
        _wrap(2.0 * math.pi * (novelty - 0.5)),
        _wrap(math.pi * math.tanh(valence)),
    ]
    return np.asarray(phases, dtype=float)
