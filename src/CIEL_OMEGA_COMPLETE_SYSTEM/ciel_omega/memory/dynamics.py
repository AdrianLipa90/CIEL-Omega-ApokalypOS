...
class MemoryDynamicsEngine:
    ...
    def step(self,
        ...
        # Berry phase accumulation: identity drifts toward mean channel phase.
        # Uses circular mean (complex exponential) to handle 2π wrap correctly.
        # Rate = 0.005 rad/unit (much slower than fastest channel τ=0.1).
        # Implements: φ_id(t+dt) = φ_id(t) + k·sin(φ_mean − φ_id)·dt
        _IDENTITY_DRIFT_RATE = 0.005
        mean_channel_phase = float(np.angle(np.mean(np.exp(1j * new_phases))))
        identity_drift = _IDENTITY_DRIFT_RATE * np.sin(mean_channel_phase - state.identity_phase)
        new_identity_phase = (state.identity_phase + identity_drift * dt) % (2 * np.pi)

        # Sync identity field so potential engine sees updated attractor
        self.identity.phase = new_identity_phase
        ...
        new_state = MemorySystemState(
            ...
            identity_phase=new_identity_phase,         # Berry phase accumulation
