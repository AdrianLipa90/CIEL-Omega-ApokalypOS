...
def bounded_holonomic_coherence(r_h: float) -> float:
    """Legacy local coherence gate for already-normalized R_H.

    In this module R_H is analytically normalized as |Δ_H|²/N², so the
    coherence complement is intentionally bounded to [0, 1].  This helper
    makes that local convention explicit and prevents it from being confused
    with the raw pipeline R_H defect, which must use ``rh_defect_score`` /
    ``rh_coherence_score``.
    """
    return max(0.0, min(1.0, 1.0 - float(r_h)))
...
def V_information(gamma: np.ndarray, gamma_truth_target: float) -> float:
    ...
    V_I = λ · C_H · (1 - cos(γ_T - γ_target))

    Pochodzenie: iloczyn dwóch miar na okręgu.
    C_H = bounded_holonomic_coherence(R_H) ∈ [0,1] — jawna bramka koherencji dla lokalnie znormalizowanego R_H
    ...
    """
    r = R_H(gamma)
    c_h = bounded_holonomic_coherence(r)
    gamma_T = gamma[-1]  # γ_T is always the last phase
    return c_h * (1.0 - np.cos(gamma_T - gamma_truth_target))


def dV_I_dgamma(gamma: np.ndarray, gamma_truth_target: float) -> np.ndarray:
    """∂V_I/∂γ_k — analitycznie.

    V_I = C_H · (1 - cos(γ_T - target))
    gdzie C_H = bounded_holonomic_coherence(R_H) dla lokalnie
    znormalizowanego R_H ∈ [0,1].

    ∂V_I/∂γ_k = −(∂R_H/∂γ_k) · (1 - cos(γ_T - target))     [k ≠ T]
               + C_H · sin(γ_T - target)                   [k = T, dodatkowy]
    """
    r = R_H(gamma)
    ...
    grad[-1] += c_h * sin_term  # dodatkowy człon dla γ_T
    return grad
