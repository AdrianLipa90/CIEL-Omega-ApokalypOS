...
def _resolve_project_root_from_metrics(repo_root: Path) -> Path:
    current = repo_root.resolve()
    for candidate in [current, *current.parents]:
        if (candidate / 'integration' / 'reports').exists() and (candidate / 'src').exists():
            return candidate
    return repo_root.resolve()


def latest_nonlocal_eba_observables(repo_root: Path) -> dict:
    """Load latest-known nonlocal/EBA + Euler bridge observables from bridge reports.

    These are runtime observables already canonically produced by the active bridge/pipeline.
    Orbital diagnostics can then carry them as part of the current world-state picture instead
    of treating them as an external side-channel.
    """
    project_root = _resolve_project_root_from_metrics(repo_root)
    report_path = project_root / 'integration' / 'reports' / 'orbital_bridge' / 'orbital_bridge_report.json'
    default = {
        'observables_present': False,
        'phi_ab_mean': 0.0,
        'phi_berry_mean': 0.0,
        'eba_defect_mean': 0.0,
        'nonlocal_coherent_fraction': 0.0,
        'bridge_closure_score': 0.0,
        'bridge_target_phase': 0.0,
        'source': 'missing',
    }
    ...
    try:
        ...
        broken = dict(default)
        broken['source'] = 'unreadable'
        return broken

    ciel = payload.get('ciel_pipeline', {}) or {}
    runtime_gating = payload.get('runtime_gating', {}) or {}
    euler_bridge = runtime_gating.get('euler_bridge', {}) or {}
    euler_metrics = euler_bridge.get('euler_metrics', {}) or {}

    return {
        'observables_present': True,
        'phi_ab_mean': float(ciel.get('phi_ab_mean', 0.0)),
        'phi_berry_mean': float(ciel.get('phi_berry_mean', 0.0)),
        'eba_defect_mean': float(ciel.get('eba_defect_mean', 0.0)),
        'nonlocal_coherent_fraction': float(ciel.get('nonlocal_coherent_fraction', 0.0)),
        'bridge_closure_score': float(euler_metrics.get('closure_score', ciel.get('bridge_closure_score', 0.0) or 0.0)),
        'bridge_target_phase': float(euler_metrics.get('target_phase', ciel.get('bridge_target_phase', 0.0) or 0.0)),
...
def A_numpy(system: OrbitalSystem) -> np.ndarray:
    """Fully vectorized A matrix — no Python loops over pairs.

    Replaces 36 scalar _complex_coupling calls with batch numpy operations.
    Each factor (tau_factor, poincare_distance, berry_phase) computed as (N×N) arrays.
    """
    names = system.names()
    n = len(names)
    if n == 0:
    ...
    sigma = float(system.params.get('sigma', 0.28))
    beta  = float(system.params.get('beta', 0.9))
    gamma = float(system.params.get('gamma', 0.25))
    mesh_boost = float(system.params.get('mesh_boost', 1.0))

    # --- extract sector arrays ---
    tau   = np.array([system.sectors[n_].tau for n_ in names])
    theta = np.array([system.sectors[n_].theta for n_ in names])
    phi   = np.array([system.sectors[n_].phi + system.sectors[n_].berry_phase for n_ in names])
    ...
    for i, a in enumerate(names):
        for j, b in enumerate(names):
            if i != j:
                base[i, j] = system.coupling(a, b) * mesh_boost

    # zero diagonal → A_ii = 0 by definition
    np.fill_diagonal(base, 0.0)
    nonzero = base > 0.0  # mask — skip zero couplings

    # --- tau_factor (N×N): exp(-0.5 * (log(tau_i/tau_j) / sigma)^2) ---
    with np.errstate(divide='ignore', invalid='ignore'):
        log_ratio = np.log(np.maximum(1e-9, tau[:, None] / tau[None, :]))
    tau_factor = np.exp(-0.5 * (log_ratio / sigma) ** 2)

    # --- poincare_distance (N×N) ---
    rho = np.minimum(0.999999, np.abs(np.tanh(np.tan(theta / 2.0))))
    dphi_pd = phi[None, :] - phi[:, None]  # (N,N): phi_j - phi_i
    num = rho[:, None]**2 + rho[None, :]**2 - 2.0*rho[:, None]*rho[None, :]*np.cos(dphi_pd)
    den = np.maximum(1e-12, (1.0 - rho[:, None]**2) * (1.0 - rho[None, :]**2))
    d_ij = np.arccosh(np.maximum(1.0, 1.0 + 2.0 * num / den))

    # --- berry_pair_phase (N×N) ---
    dphi_bp = (phi[None, :] - phi[:, None] + np.pi) % (2.0 * np.pi) - np.pi
    avg_theta = 0.5 * (theta[:, None] + theta[None, :])
    omega_ij = 0.5 * (1.0 - np.cos(avg_theta)) * dphi_bp

    # --- assemble A ---
    phase = beta * omega_ij - gamma * d_ij
    A = (base * tau_factor) * np.exp(1j * phase)
    A[~nonzero] = 0.0  # zero where no coupling
